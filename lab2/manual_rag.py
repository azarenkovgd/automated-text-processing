...
def create_query_function_rag_manual(dataset: list[dict]):
    index = create_faiss_index(dataset)

    return lambda query: search_in_dataset(query, index, dataset)
...
def main():
    ...
    with open('data/documentation.json', 'r', encoding="utf-8") as file:
        dataset = json.load(file)

    print_response("Give me minimal example of the bot setup for python-telegram-bot", dataset)
